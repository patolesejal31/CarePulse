...
export const getRecentPatientAppointments = async (userId: string) => {
  try {
    const appointments = await databases.listDocuments(
      DATABASE_ID!,
      APPOINTMENT_COLLECTION_ID!,
      [
        Query.equal("userId", userId), // Filtering by the patient userId
        Query.orderDesc("$createdAt"),  // Ordering by creation date
      ]
    );
...
export const getPatientAppointmentStats = async (userId: string) => {
  try {
    const appointments = await databases.listDocuments(
      DATABASE_ID!,
      APPOINTMENT_COLLECTION_ID!,
